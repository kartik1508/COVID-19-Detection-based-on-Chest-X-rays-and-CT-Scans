...
def uploaded_chest():
   ...
   mobile_net_chest= load_model('models/mobile_net.h5')
   ...
   mobilenet_pred = mobile_net_chest.predict(image)
   probability = mobilenet_pred[0]
   print("Mobile_net Predictions:")
   if probability[0] > 0.5:
      mobilenet_chest_pred = str('%.2f' % (probability[0]*100) + '% COVID') 
   else:
      mobilenet_chest_pred = str('%.2f' % ((1-probability[0])*100) + '% NonCOVID')
   print(mobilenet_chest_pred)

   return render_template('results.html',resnet_chest_pred=resnet_chest_pred,vgg_chest_pred=vgg_chest_pred,inception_chest_pred=inception_chest_pred,xception_chest_pred=xception_chest_pred,mobilenet_chest_pred=mobilenet_pred)
